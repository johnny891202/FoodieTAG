...
newTag.addEventListener('input', e => {
    ...
    if (newTagValue !== "") {
        ...
        tagData.forEach(function (item) {
            if (item.TagName.includes(newTagValue)) {
                str += `<a href="#"><li class="tag me-4 mb-2 bg-primary-400 text-white" id="tags" value-id="${item.TagName}">${item.TagName}</li></a>`
            }
        })
        recommendTagsContainer.innerHTML = str;
    }
    // tagData.forEach(item => {
    //     // console.log(item)
    //     if (item.TagName.includes(newTagValue)) {
